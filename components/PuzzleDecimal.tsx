...
export default function PuzzleDecimal({
  ...
  // Converte a entrada para número e compara com o objetivo
  if (parseInt(inputValor) === numeroObjetivo) {
    setSucesso(true);
    setErro(false);
    
    // O finalizarFase calcula o tempo e envia os dados para a Store
    // Passei 40 como pontuação base, mas você pode alterar
    setTimeout(() => finalizarFase(40), 1500); 
    
  } else {
    // Registra a tentativa incorreta na telemetria
    registrarErro();
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-4 md:grid-cols-8 gap-2 md:gap-3">
        {bitsGerados.map((bit, i) => (
          <div key={i} className="flex flex-col items-center gap-2">
            ...
            <span
              className={`text-[9px] font-mono font-bold transition-opacity duration-500 ${
      ...
      <div className="text-[10px] text-green-900 uppercase font-medium h-4">
        {!sucesso &&
          mostrarDica &&
          "Dica: Some os pesos superiores dos bits ativos."}
