...
export default function PuzzlePotencias({ numeroObjetivo, onAcerto }: Props) {
  ...
  const valoresDecimais = useMemo(() => pesos.map(p => Math.pow(2, p)), [pesos]);

  const somaAtual = bits.reduce((acc, bit, i) => (bit === 1 ? acc + valoresDecimais[i] : acc), 0);
  ...
  }, [somaAtual, numeroObjetivo, sucesso, onAcerto]);

  return (
    <div className={`flex flex-col items-center gap-6 p-6 border-2 transition-all duration-500 rounded-lg ${
      sucesso ? 'border-green-400 bg-green-500/10' : 'border-green-900 bg-zinc-900/30'
    }`}>
      ...
      <div className="text-center space-y-1">
        ...
        <h2 className={`text-4xl font-black transition-colors ${sucesso ? 'text-green-400' : 'text-white'}`}>
        ...
        {pesos.map((p, i) => (
          <div key={i} className="flex flex-col items-center gap-2">
            ...
            <div className="flex text-green-500 font-mono text-sm group">
              <span>2</span>
              <span className="text-[10px] -mt-1 font-bold group-hover:text-white transition-colors">{p}</span>
            </div>
            ...
            >
              ...
              {bits[i] === 1 && (
                <span className="text-[9px] font-bold mt-1 opacity-70">({valoresDecimais[i]})</span>
              )}
            </button>
          </div>
        ))}
      ...
      <div className="w-full h-2 bg-zinc-950 rounded-full overflow-hidden border border-green-900/30">
        <div 
          ...
          style={{ width: `${Math.min((somaAtual / numeroObjetivo) * 100, 100)}%` }}
        />
      </div>
